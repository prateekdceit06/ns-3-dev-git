    ...
    static TypeId tid =
        TypeId("ns3::QuicEchoClient")
            ...
            .AddAttribute(
                ...
                UintegerValue(100),
                MakeUintegerAccessor(&QuicEchoClient::GetDataSize, &QuicEchoClient::SetDataSize),
                MakeUintegerChecker<uint32_t>())
            .AddAttribute(
                "StreamId",
                "Identifier of the stream to be used in the QUIC connection",
                UintegerValue(2),
                MakeUintegerAccessor(&QuicEchoClient::GetStreamId, &QuicEchoClient::SetStreamId),
                MakeUintegerChecker<uint32_t>())
            .AddTraceSource("Tx",
...
QuicEchoClient::Send(void)
{
    ...
    if (m_dataSize)
    {
        ...
        NS_ASSERT_MSG(m_dataSize == m_size,
                      "QuicEchoClient::Send(): m_size and m_dataSize inconsistent");
    ...
    NS_ASSERT_MSG(sent - p->GetSize() == 0,
                  "Could not send data (packet and sent size inconsistent)");
    ...
    {
        NS_LOG_INFO(
            "At time " << Simulator::Now().GetSeconds() << "s client sent " << m_size
